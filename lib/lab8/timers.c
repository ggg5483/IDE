...
*/
static void TIMG_power(GPTIMER_Regs *TIM){
...
void TIMA0_PWM_DutyCycle(uint8_t pin, double percentDutyCycle){
    TIMA0->COUNTERREGS.CC_01[pin] = (uint32_t) ((1.0 - percentDutyCycle) * (double)TIMA0->COUNTERREGS.LOAD);
}
...
void TIMA1_PWM_DutyCycle(uint8_t pin, double percentDutyCycle){
    TIMA1->COUNTERREGS.CC_01[pin] = (uint32_t) ((1.0 - percentDutyCycle) * (double)TIMA1->COUNTERREGS.LOAD);
