...
void UART0_init(void){
    ...
    double clk = (double)CLK_RATE;
    clk = clk / ((double)SAMPLING * (double)BAUD_RATE);
    UART0->IBRD = (uint32_t) clk;
    
    double frac = clk - (double)((uint32_t)clk);
    UART0->FBRD = (uint32_t)((frac * 64.0) + 0.5);
...
void UART1_init(void){
    ...
    //set baud rate 9600
    //BRD = UART Clock / (Oversampling x Baud rate) 
    double clk1 = (double)CLK_RATE;
    clk1 = clk1 / ((double)SAMPLING * (double)BAUD_RATE);
    UART1->IBRD = (uint32_t) clk1;
    
    double frac1 = clk1 - (double)((uint32_t)clk1);
    UART1->FBRD = (uint32_t)((frac1 * 64.0) + 0.5);
